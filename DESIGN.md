...
| Birthday       | a cake beside the bar      | Aqua Glaciar   | 10.02:1        |
...
- **The birthday is not an event, so it is not a corner.** The two corners hold things somebody logged; a birthday is what the date _is_. It shares the strip under the number with the goal bar, side by side when a day carries both — which is also the only space a 10dp glyph fits, because the strip is reserved on every cell at a constant height and 28 + 2 + 10 is 40dp of a 42.5dp cell, measured. **It keeps Aqua Glaciar rather than taking a new hue**: the palette has no colour left that is neither an alarm nor an instruction, and a cake is a shape nothing else here resembles, so the hue does not have to do the work. By The Frequency-Before-Hue Rule it could afford the loudest treatment on the calendar — once a year against the goal's eighteen days a month — and it does not need it.
