...
- **An icon is not a reason.** The alert state says something failed; the screen still owes the user a sentence saying what. Never let a throw escape a handler and leave a red flash as the only explanation.
...
- **Don't** let the warm voice reach an error. A failure needs to be unambiguous, so wit is a liability there: state what went wrong, take responsibility for it when it is ours ("Algo ha ido mal por nuestro lado"), and offer the way out. Friendly and clear do coexist in one place — an unfilled field can be asked about rather than legislated at, so "¿Cómo se llama?" replaces "El nombre es obligatorio" and says the same thing.
