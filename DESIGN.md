...
- **The ideal is marked, and it had to be.** A five-point scale reads as one-ended — the assumption is that the best is whatever is furthest from diarrhoea — and the person who designed this scale read it that way himself. A sentence under the row would be read once and never again; a mark is there every time. The healthy step carries a stronger hairline and its word in the primary tone, and says "Es la buena" in its accessible name. One step up in weight, no hue, no badge, the other four untouched.
- **On the row, right of everything else**, one small token per stool with its number. The walk's own facts read first; this is what it left behind, and it is decoration to a screen reader because the row's accessible name already says it in words.
