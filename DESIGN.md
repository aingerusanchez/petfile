...
- **The scrim is a sibling of the panel, not its parent.** It wrapped it until the scrim gained the accessible name it needed: a `Pressable` with a button role renders as a real `<button>` on the web, and every control in the panel became a button inside a button — a hydration warning per sheet, and a thing **only the web can see**, since React Native has no rule against a pressable inside a pressable. Side by side the nesting cannot happen and the panel needs no `stopPropagation` to survive a tap.
...
- **The calendar reads a window of months, not the month on screen.** The library's header arrows change the month by dispatching their own state and calling nobody — `onMonthChange` fires from the month _list_ alone — so a per-month fetch could not know it had to run again. Paging back one month left every cell in the unlogged branch, with the calendar asserting that nothing happened all August. One request covering a year of one dog's entries is cheap, and it is the honest fix: past the window's edge the marks stop, which is a bounded limit rather than a lie that moves with you.
- **A month that could not be read says so.** An empty map and a quiet month render identically — dimmed numbers, "sin registros" — so a timed-out read had the calendar reporting a month of nothing. For a product whose promise is never losing a record, inventing the loss is worse than admitting the failure.
- **No goal, no legend row for it.** The legend named "Objetivo conseguido" even when `exercise_goal_minutes` is null and the bar can never be drawn — a promise the grid cannot keep.
...
- **The month's cells clear 48dp on both axes, and the second one was nearly written off.** The rows come from `containerHeight`, which the library divides by six. The columns looked like geometry — 44.6dp, seven of them, "a phone is not wide enough" — and that was wrong: a column _is_ 48.1dp, and `classNames.day_cell`'s `p-0.5` was taking 1.75dp off each side of the pressable inside it. The padding bought nothing, because the cell's content is a centred chip either way. Measured on the device, on both calendars: 44.6 × 42.5 before, 48.3 × 52.6 after. **The lesson is the general one: measure the target, not the column.**
...
- **One accent answers "which day", the other answers "how did it go".** Ice Blue Glacial fills the selected day, outlines today, and draws the border of the button that goes back to today — three marks about _where you are and where you can go_, and the last two share a subject, so sharing a colour is the point rather than a coincidence. Aqua Glaciar is left to mean exactly one thing on this grid: the goal, met. The split was forced by a collision — the ring reached for the secondary first and the calendar then wore aqua for two unrelated ideas — and the app's text actions are Aqua Glaciar, which is why the way back takes the primary on its **border** and leaves its word alone.
- **The vaccine borrows the birthday's slot and yields it.** Both are watermarks behind the number rather than corner marks, because both are what the _date_ is rather than something somebody logged that day — and both are about annual, so a cell carrying the two is a coincidence worth a year of waiting. The cake wins it. At 26 rather than the cake's 32: a syringe is a long diagonal and at 32 its barrel runs straight under the day's own number, where the cake's mass sits below it. **Nothing else from Salud earns a cell** — a monthly antiparasitic would be twelve marks a year, and a weight one a fortnight, on a grid whose marks mean "the exception".
- **The ring changed colour so the cake could change size.** It was a Steel Frost hairline, and that hairline held the birthday cake down to 22dp — big enough to clear the ring, too small to be recognised as a cake at all. Two concentric grey circles crossing each other is a scribble; two shapes in different hues crossing each other is two shapes. So the ring took the accent and the cake took the size (32dp, spanning 8 to 40 across a 48dp cell, clear of the corner marks at 10). It also made today **visible**, which the hairline barely was.
- **The cake is drawn twice on the day it belongs to.** The calendar opens on today, so on the birthday itself the selected day's accent fill covered the watermark completely — the one cell the whole feature exists for was the one cell with no cake. A second copy sits over the fill in the number's own dark ink at a fifth opacity, clipped by the chip, so the part that would land on the panel as invisible ink never draws. Both copies share a centre — the watermark's box excludes the bar strip precisely so that it does — and the result reads as one cake that changes tone where it crosses the circle.
...
What the dog left on the walk, on a five-point scale, behind one button in the walk's own sheet.

- **The button lives in the note's own row, because typing "💩" into the note is the habit it replaces.** It costs the walk no vertical space at all until somebody reaches for it, and it is the walk's alone — the other three kinds are a moment, not an outing.
- **The button reveals; the palette adds.** Adding first and choosing after would need a default, and a default is a fabricated value one careless tap away — the same reason the walk proposes no duration. Tapping an option appends it **and leaves the palette open**, so the second stool costs one more tap. One is two taps; none is zero.
- **The scale runs backwards, 5 to 1.** Read left to right that is the wrong way round, and it puts "Perfecta" — the one tapped on almost every walk — under the thumb instead of across the phone from it. The order still carries the scale; it just runs toward the hand.
- **Five in one row, never wrapped**, growing to share the width and shrinking to fit. A wrapped row moves the options between taps, and this is a control tapped two or three times in a row.
- **What grows goes above what repeats.** A bottom sheet grows upward, so anything appearing below the palette lifts it out from under the thumb aiming at it: measured at 110dp on the device, with the next tap landing on the control that had taken its place. The collected ones therefore sit between the note and the palette. Verified on hardware: three adds from the same spot, the palette identical to the pixel.
- **Tapping a collected one removes it**, which is the whole repair for a mis-tap — one press, no confirmation, no menu. They are values in an unsaved form rather than stored records, so the rule that keeps delete out of a list is not what is at stake.
- **Mucus and blood are not here.** They were two checkboxes for one build, and that was friction on the path everybody takes for the sake of the one they almost never do. Something worth a vet's attention is an incident today, and the health tab when it exists.
- **No colour ramp.** Five steps invite a traffic light, and red already means an incident and amber medication on the calendar. Consistency is not an alert.
- **The ideal is marked, and it had to be.** A five-point scale reads as one-ended — the assumption is that the best is whatever is furthest from diarrhoea — and the person who designed this scale read it that way himself. A sentence under the row would be read once and never again; a mark is there every time. The healthy step carries a stronger hairline and its drawing in the primary tone, and says "Es la buena" in its accessible name. One step up in weight, no hue, no badge, the other four untouched.
- **Each step is a drawing, not a digit.** What the scale measures is _how the mass held together_, which is a picture: separate pellets, one segmented log, that same log with its edges gone, a mass spread flat and torn, a puddle with something still dropping into it. The words survive where there is room to read them — on a collected one, and in every accessible name. `components/ui/StoolMark.tsx`.
- **The collected ones are the legend, and they teach without a legend's cost.** A drawing nobody has met yet is ambiguous by definition, and five of them at once are five guesses. But a tap puts the chosen one directly above the palette **with its word**, while the palette stays open — so the first uncertain tap answers itself, in the same place, without interrupting the adding. No key, no tooltip, no first-run overlay: the vocabulary is learnt by using it, once, and the words are gone from the row that gets tapped every day.
- **They are drawn in Lucide's idiom because they sit beside Lucide's icons**: a 24-unit grid, a 2-unit stroke, round caps and joins, no fills, and the colour arriving as a prop. A mark from another family reads as a sticker dropped on the form.
- **18dp is the size that decides, and four drafts died there.** The marks are read at 26 in the palette and at 20 on the log's row, where detail is gone and only the silhouette survives — so the five differ by _outline_, never by ornament. Two early drafts (a bumpy mound for "blanda", a ruffled one for "sin forma") both rendered as **clouds**, and as each other: rounded bumps were the cause, so one lost them and the other took angular tears instead. Diarrhoea's first draft put two matched drops over the puddle and read as a **face** at every size; one large drop and one small, off-centre, breaks the symmetry that made eyes. Every candidate was rendered side by side at 72/40/26/18 before one was chosen.
- **`transform="rotate(…)"` as a string, not `rotation`/`originX`.** The prop form is what react-native-svg documents, and on the web it emits a `transform-origin` **attribute** that React rejects outright: `Invalid DOM property`, once per render, in an app whose console is otherwise clean. The string form renders the same rotation on both targets and says nothing.
- **On the row, right of everything else**, one 20dp mark per stool. The walk's own facts read first; this is what it left behind, and it is decoration to a screen reader because the row's accessible name already says it in words.
...
One of two, in a single joined track, and the chosen one inverts.

- **A subtle segmented control is not available in this palette, and that is measured.** Elevated Frost on Fjord Slate is 1.16:1 — the same imperceptible step that made the chips carry a weight change instead of a fill — so a quiet track with a quietly raised segment would read as two words floating in a box. The selected segment takes Ice Blue Glacial with an `on-accent` label: a luminance inversion nobody can miss, which survives colour blindness and daylight alike and therefore satisfies the rule against state carried by hue.
- **It is not a new idea, which is the point.** The calendar's selected day and the clock face's chosen number are already a filled Ice Blue Glacial with dark text. This is the third place the app says "the chosen one is the filled one"; a third dialect for the same sentence would have been the defect. The One Accent Rule is about actions, and all three of these mark a value.
- **Joined, unlike `ChipGroup`.** Separate chips are a set of options; a track with a divide down it is one setting with two sides. **The fill boundary is the divider** — exactly one segment is ever filled — so nothing is drawn inside the track. `overflow-hidden` on the track is what clips an end segment's fill to the rounded corner.
- **Full width, split evenly.** Sized to their labels the two sides come out different widths, which reads as two buttons that happen to touch; an even split reads as one track with a position in it. What makes the full width affordable is the inversion itself — a filled segment says "state" where two bordered pills across a card said "two large buttons".
- **An icon per segment, and the pair has to be one contrast rather than two of a kind.** A stethoscope for the routine clinic and a siren for the emergency: routine care against an emergency light, which is the one glyph on a dark screen nobody has to decode. Both labels are always on screen, so nothing is a rebus — the glyph is what lets an eye find the emergency side without reading either. **It cost the person row its own stethoscope**, which was the same glyph meaning "the vet you ask for" six lines below; a named person is a `User`.
...
- **The selector is a `Segmented` track, and the live side inverts.** Two bordered chips kept both labels on screen but said it as two large buttons, the card's main event, when what they are is one setting with two sides. They stay a pressable block rather than becoming underlined links: a tab that looks like a section heading is a tab nobody presses, and this app's section headings are exactly that shape.
...
- **The schedule is a textarea, and one line per day.** "L-V 10:30-14:00, 15:00-19:30, S 10:00-13:00" is a single line that has to be parsed by eye every time it is read; the same thing down the page is scanned. It takes the same Markdown the notes take, with the same help in its corner — a dash per day is a list — and the clock glyph aligns to the first line rather than floating beside Wednesday.
...
- **A lone newline is a soft wrap, except where it cannot be.** A note typed on a phone wraps by accident, so a single newline joins — which is what Markdown itself does. A clinic's opening hours are the opposite: every line is a different day, nobody breaks one by mistake, and joining them produces a sentence about nothing. `breaks` is the exception, set per field rather than guessed from the text, and GitHub-flavoured Markdown makes the same one.
...
- **The month label yields to whatever the caller anchors in the corner.** The section puts a maximise mark bottom-right and the last month was sitting underneath it; `endInset` stops the label short by exactly that much, rather than moving the mark somewhere it means less. A line with an empty corner gives the month its width back.
...
- **The sections read and the floating action writes.** Each section carried its own add button, which put the one thing a tutor comes here to do at the foot of a block they had to scroll to, and made a list of records look like a form. The diary had already settled the shape: two actions behind the floating button, weight nearest the thumb because it is the one logged between vet visits.
- **The glyph leads a treatment row as well as its chip.** A year of monthly antiparasitics is fifteen rows opening with the same three words; the icon is what lets an eye find the vaccine among them without reading one.
...
- **The field stays typed and the clock is the alternative.** A time is faster to type than to browse — "915" is two seconds — so typing keeps the whole width and this is a glyph beside it. It is here for the tutor who would rather not aim at a number pad, and for the one who cannot: a 48dp target is a far easier one than four digits.
- **A clock face, because nobody has to be taught one.** The first version was a strip of hour chips over a grid of minutes: correct, legible, and a control that exists nowhere else in the world. A face with a hand on it carries its own instructions, and it answers "morning or evening" with its shape before any digit is read.
- **One ring and a switch, not two rings.** Material's 24-hour dial nests a second ring inside the first, which on this phone puts its numbers 38dp apart — under the floor every other control here is held to. So the face carries twelve marks at 54dp of separation and a two-way switch says which half of the day they are labelled with. **The hand does not move when the half changes**, because 09 and 21 are the same place on a clock; the switch reads as relabelling rather than as a jump, and it stays live while the minutes are being set, where it still means "the other half of the day".
- **Tap a number or drag the hand.** The numbers are real buttons — what a screen reader needs and what the suite drives — and the drag snaps to the minute. Choosing an hour hands over to the minutes: the two are one gesture, not two errands. The gesture also refuses to give itself back (`onResponderTerminationRequest`), because a circular drag has a vertical component and the sheet it sits in scrolls.
- **The responder props rather than a `PanResponder`.** A `PanResponder` has to be built once, so its handlers cannot see state and have to read it back out of refs — which the React Compiler rules reject on sight. Plain responder props are rebuilt every render on a node that is not, so they simply close over the state they need.
- **An off-grid minute is kept, not rounded.** Opening on 09:47 puts the hand on 47 and confirming without touching anything gives back 09:47. Rounding on open would be the picker quietly editing a value nobody asked it to touch.
- **A number gets the disc; a gap gets a point.** Measured on the device at 18:07: the full knob sat half over the "05" and read as though five were chosen and smudged. The hand's tip shrinks to a dot when it lands between two numbers, which is also the honest difference between "this one" and "between these".
...
- **The picker's heading is a prop, and it used to be a hard-coded string.** "Fecha de nacimiento" was correct for the only field that existed, and the first treatment that borrowed the component opened a sheet titled with the wrong subject. The label above the field is shouted in caps and cannot double as a heading, so each call site says what it is asking: "¿Cuándo toca la siguiente?" reads as a question the picker answers, where "PRÓXIMA" reads as a column header.
- **The field is typed and the picker is behind its own glyph.** Every date went through three taps in a calendar, including the ones somebody already knew — a birth date four years back is a year of paging. The text area now takes `DD/MM/AAAA`, or the eight digits a number pad gives, and the icon opens the calendar for the dates that are easier to point at than to spell. **No live mask**: inserting the slashes as the digits arrive was built, tested and removed here for the time fields, because a focused `TextInput` on Android ignores a value the JS layer rewrites. It normalises on blur, which is the correction the platform honours — and the same bargain Ajustes already advertises for times. Approximate mode keeps the single button: a month and a year are two choices off a grid with nothing to type.
- **The picker's answer replaces what was typed even when it agrees with what was stored.** Resyncing only on a changed value left a refused date sitting in the field, with its error under it, after the tutor had gone to the calendar and chosen the date that was already saved.
...
- **The fortnight before is a countdown, under the file's three lines**: "Quedan 15 días para su cumpleaños", and "Mañana es su cumpleaños" on the last one — the same instinct as "Hoy" and "Ayer" in the day headline. **The two never share the screen**: at zero days the age line itself is rewritten, so the arrival is a change of voice and not one more line.
- **It is Aqua Glaciar, and it was Mist Grey first.** Grey put it a step under the age line above it and it vanished into it — the wrong outcome for something that shows on fifteen days of the year. The secondary accent is already this screen's colour for the birthday, so before and on the day it is one subject in one colour, and on a profile whose only other hues are the dog's own photo it is the thing the eye finds.
- **Regular weight, and that is what keeps it honest.** `Button variant="link"` is aqua _semibold_, and the link it would otherwise impersonate — "Completa su ficha" — sits in this very block. Weight is what separates "notable" from "tappable" once they share a colour. The countdown also carries **no cake**: the glyph belongs to the day itself.
