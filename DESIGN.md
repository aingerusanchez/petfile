...
- **A 56dp square at the system radius, not a circle.** Material 3 draws square FABs too, so nothing is being fought; The One Radius Rule keeps its two exceptions.
...
- **The duration also has -15 / +15.** The fastest walk to log is the one with nothing to say about it: tap Paseo, tap +15 three times, save. The steppers write into HASTA exactly as typing a duration does, so the rule above holds whichever control moves. `-15` is disabled at or below fifteen minutes rather than clamping to zero, because a zero-minute walk is not a shorter walk. The field is only as wide as a three-digit value needs, which is what leaves room for them.
