...
- **The month's rows are 48dp because the container says so.** Seven columns in a phone's width cannot also be 48 wide — 44.6dp is what 375dp leaves after the panel's own padding, and reaching 48 would mean a calendar flush to both screen edges. Material's own date picker makes the same trade: the row clears the floor, the column does not, and the axis a thumb actually misses on is the vertical one. Measured on the device: 48.9dp tall after the container grew, 42.5 before.
