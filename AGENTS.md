...
- **Delete the pre-MVP paragraph at the top of `CHANGELOG.md` when the version reaches `v1.0.0`.** It explains a rule that stops existing at that moment, and the file carries an HTML comment saying so. The app's renderer skips those comments, so notes to editors can live in the file without reaching a reader.
- **Every feature and every bugfix gets a version, and `CHANGELOG.md` is where they are written down.** Bump `package.json` as part of the work, not afterwards, and add the entry in the same commit — the file is the one place that says what a build on somebody's phone actually contains. Keep the voice of the other docs: say what changed and why it was wrong, not "fixed a bug". Versions recorded in retrospect say so.
...
**`react-native-ui-datepicker`'s header arrows change the month and tell nobody.** They call its internal `onChangeMonth`, which dispatches state; the `onMonthChange` prop fires only from the month _list_. Anything that has to stay in step with the displayed month therefore cannot be fetched per month — the day view reads a window of months instead. Its `month`/`year` props do not rescue this either: they are folded into state built once, and setting them left the grid on the old month (measured).

**`classNames.day_cell` defaults to padding that eats the touch target.** `p-0.5` is 1.75dp a side on native, which took a 48.1dp column down to a 44.6dp pressable on both calendars — 60 targets under Android's floor. The cell's content is centred either way, so the padding costs the floor and buys nothing.

**A custom `components.Day` replaces the library's cell content**, so `classNames.selected`, `classNames.today` **and `classNames.day` itself** never reach it — the library applies that className only in its default branch and hands the custom branch a `style` alone. The last one costs the touch target: `calendar.ts` writes `min-h-[48px]` there and 30 day cells still measured 42.5dp. Nor can the cell be grown from the inside, because its `Pressable` is `flex: 1` inside a parent whose height is a minimum, so content never pushes it. **The only lever is the `containerHeight` prop**: `dayCell.minHeight = (containerHeight - 25) / 6`, less the cell's `p-0.5`. The default 300 gives 42.3dp; `MonthCalendar` passes 340 for 49. A calendar with a custom day has to draw its own selection and size its own rows. It is also wrapped in the library's own `Pressable` with a hardcoded `accessibilityLabel` of the day number and no prop to override it, so a richer label has to sit on a node inside that wrapper — which makes every day **two** TalkBack stops. Measured on the device: both nodes are focusable with identical bounds.
...
| Any user-visible feature or bugfix     | `CHANGELOG.md`, in the same commit, with the `package.json` bump                 |
